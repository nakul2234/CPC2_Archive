...
def calculate_potential(fasta,strand,outfile):
	...
	ftmp_result.write("\t".join(map(str,["#ID","transcript_length","peptide_length","Fickett_score","pI","ORF_integrity","start_codon_position","coding_probability","label"]))+"\n")
	ftmp_result.close()
	fickett_obj = Fickett()
	for seq in seqio.fasta_read(fasta):
		...
		ftmp_feat.write("\t".join(map(str,[seqid,len(seqRNA),pep_len,fickett_score,isoelectric_point,orf_fullness,start_pos]))+"\n")
